...
function aiomatic_replaceAIExecludes($article, &$htmlfounds, $opt = false, $dymmy_char = '-')
{
    $orig_trans = $article;
    $htmlurls = array();
    $article = preg_replace('{data-image-description="(?:[^\"]*?)"}i', '', $article);
    if($article === null)
    {
        $article = $orig_trans;
    }
    ...
    try {
        ...
        elseif(strstr($to, '-') !== false && $to != 'zh-CN' && $to != 'zh-TW')
        {
            ...
            else
            {
                ...
                if(!isset($trans_json['translations'][0]['text']))
                {
                    throw new Exception('Unrecognized text response from DeepL: ' . 'https://api.deepl.com/v2/translate?text=' . urlencode($content) . '&source_lang=' . $from . '&target_lang=' . $to . '&auth_key=' . trim($aiomatic_Main_Settings['deepl_auth']) . '&tag_handling=html&preserve_formatting=1' . ' --- ' . $translated);
                }
                $translated = $trans_json['translations'][0]['text'];
            }
            ...
            if($trans_json === null)
            {
                throw new Exception('Incorrect text response from DeepL: ' . $translated_title);
            }
            if(!isset($trans_json['translations'][0]['text']))
            {
                throw new Exception('Unrecognized text response from DeepL: ' . 'https://api.deepl.com/v2/translate?text=' . urlencode($content) . '&source_lang=' . $from . '&target_lang=' . $to . '&auth_key=' . trim($aiomatic_Main_Settings['deepl_auth']) . '&tag_handling=html&preserve_formatting=1' . ' --- ' . $translated);
...
function aiomatic_wordai_spin_text($title, $content)
{
    ...
    $wordai_uniqueness = '2';
    if (!empty($aiomatic_Main_Settings['wordai_uniqueness']) && in_array($aiomatic_Main_Settings['wordai_uniqueness'], ['1', '2', '3'])) {
        $wordai_uniqueness = trim($aiomatic_Main_Settings['wordai_uniqueness']);
    }
    ...
    $newhtml = $title . ' ' . $titleSeparator . ' ' . $content;
    $wordCount = str_word_count(strip_tags($newhtml));

    $sendToWordai = function($htmlChunk) use ($email, $pass, $wordai_uniqueness) {
        ...
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "input=" . urlencode($htmlChunk) . "&uniqueness=" . $wordai_uniqueness . "&rewrite_num=1&return_rewrites=true&email=" . $email . "&key=" . $pass);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 10);
    ...
    if ($wordCount > 4950) 
    {
        $chunks = [];
        $currentChunk = '';
        $words = explode(' ', $newhtml);
        foreach ($words as $word) {
            $currentChunk .= $word . ' ';
            if (str_word_count($currentChunk) >= 4500) {
        ...
        foreach ($chunks as $chunk) {
            sleep(1);
            $response = $sendToWordai($chunk);
            if (!isset($response->rewrites)) {
                aiomatic_log_to_file('"Wordai" unrecognized response on chunk: ' . print_r($response, true));
                return FALSE;
            }
            $spinned .= ' ' . $response->rewrites[0];
        }
    } else {
        sleep(1);
        $response = $sendToWordai($newhtml);
        if (!isset($response->rewrites)) {
            aiomatic_log_to_file('"Wordai" unrecognized response: ' . print_r($response, true));
            return FALSE;
        }
        $spinned = $response->rewrites[0];
    }

    $result = explode($titleSeparator, $spinned, 2);
    if (count($result) < 2) {
        $result[1] = $result[0];
        $result[0] = $title;
    }
    ...
    if(str_word_count($html) > 5000)
    {
        ...
        while($html != '')
        {
            ...
            else
            {
                ...
                if($json_res !== null && isset($json_res->text) && isset($json_res->code) && $json_res->code == 200)
                {
                ...
                {
                    $result .= $first30k;
                    aiomatic_log_to_file('Failed to spin content using SpinnerChief: ' . $json_res->text);
                }
            }
            curl_close ($ch);
        }

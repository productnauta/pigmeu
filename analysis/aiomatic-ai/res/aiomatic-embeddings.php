...
<div class="wrap gs_popuptype_holder">
        ...
        <nav class="nav-tab-wrapper">
            ...
            <a href="#tab-x" datahref="<?php echo esc_url(admin_url('admin.php?page=aiomatic_admin_settings#tab-13'));?>" class="nav-tab"><?php echo esc_html__("Settings", 'aiomatic-automatic-ai-content-writer');?></a>
        </nav>
        <div id="tab-0" class="tab-content">
         ...
         <p><?php echo esc_html__("If you are looking for more complex way to customize the AI content writer and to be able to \"teach\" the AI a large set of information (by creating your own fine-tuned model), I suggest you check the", 'aiomatic-automatic-ai-content-writer');?> <a href="<?php echo esc_url(admin_url('admin.php?page=aiomatic_openai_training'));?>"><?php echo esc_html__("AI Model Training", 'aiomatic-automatic-ai-content-writer');?></a> <?php echo esc_html__("feature of the plugin.", 'aiomatic-automatic-ai-content-writer');?></p>
...
?>
    ...
    <input type="hidden" name="nonce" value="<?php echo esc_html(wp_create_nonce('aiomatic_embeddings'));?>"> 
...
if($aiomatic_embeddings->have_posts()){
    echo '<br><br>' . esc_html__('All embeddings', 'aiomatic-automatic-ai-content-writer') . ' (' . esc_html($aiomatic_embeddings->found_posts) . ')<br>';
}
...
<table class="wp-list-table widefat fixed striped table-view-list posts">
    ...
    if($aiomatic_embeddings->have_posts()){
        foreach ($aiomatic_embeddings->posts as $aiomatic_embedding){
            ...
            ?>
            <tr>
                <td><input class="aiomatic-select-embedding" id="aiomatic-select-<?php echo esc_html($aiomatic_embedding->ID);?>" type="checkbox" name="ids[]" value="<?php echo esc_html($aiomatic_embedding->ID);?>"></td>
                <td><a href="<?php echo esc_url(get_edit_post_link($aiomatic_embedding->ID));?>" class="aiomatic-embedding-content"><?php echo esc_html($aiomatic_embedding->post_title);?></a></td>
                ...
                <td><?php if(!empty($model)){$embprice = $stats->calculatePrice($model, $token) * 2;}else{$embprice = !empty($token) ? ((int)esc_html($token)*0.0004).'$': '--';}echo esc_html($embprice);?></td>
                <td><?php echo esc_html($aiomatic_embedding->post_date);?></td>
                <td>
                <button class="button button-small" id="aiomatic_manage_embedding_<?php echo esc_html($aiomatic_embedding->ID);?>" onclick="location.href='<?php echo esc_url(get_edit_post_link($aiomatic_embedding->ID));?>';" href="<?php echo esc_url(get_edit_post_link($aiomatic_embedding->ID));?>"><?php echo esc_html__("Manage", 'aiomatic-automatic-ai-content-writer');?></button>
                <button class="button button-small aiomatic_delete_embedding" id="aiomatic_delete_embedding_<?php echo esc_html($aiomatic_embedding->ID);?>" delete-id="<?php echo esc_html($aiomatic_embedding->ID);?>"><?php echo esc_html__("Delete", 'aiomatic-automatic-ai-content-writer');?></button>
                </td>
            </tr>
            <?php
        }

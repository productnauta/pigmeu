...
<code>{"custom_id": "request-1", "method": "POST", "url": "/v1/chat/completions", "body": {"model": "gpt-4.1-mini", "messages": [{"role": "system", "content": "You are a helpful assistant."},{"role": "user", "content": "Hello world!"}],"max_tokens": 1000}}<br/>
{"custom_id": "request-2", "method": "POST", "url": "/v1/chat/completions", "body": {"model": "gpt-4.1-mini", "messages": [{"role": "system", "content": "You are an unhelpful assistant."},{"role": "user", "content": "Hello world!"}],"max_tokens": 1000}}</code>
...
{
    echo '<option value="' . esc_html($bm) . '">' . esc_html($bm);
...
<form id="aiomatic_upload_convert" class="aiomatic_none" action="" method="post">
    ...
    <input type="hidden" name="nonce" value="<?php echo esc_html(wp_create_nonce('openai-training-nonce'));?>">
...
if($aiomatic_batches->have_posts()){
    echo '<br><br>' . esc_html__('All AI Batch Requests', 'aiomatic-automatic-ai-content-writer') . ' (' . esc_html($aiomatic_batches->found_posts) . ')<br>';
}
    ...
    if($aiomatic_batches->have_posts())
    {
        foreach ($aiomatic_batches->posts as $aiomatic_batch)
        {
            ...
            <tr>
                ...
                <td><?php echo '<span '; if($status == 'failed' || $status == 'expired'){ echo 'class="cr_red"';}elseif($status == 'completed'){ echo 'class="cr_green"';}elseif($status == 'cancelled'){ echo 'class="cr_darkyellow"';} echo '>';echo esc_html($status);echo '</span>';?></td>
                <td><?php echo esc_html(get_post_meta($aiomatic_batch->ID, '_batch_endpoint', true));?></td>
                <td><?php echo esc_html($aiomatic_batch->post_date)?></td>
                <td>
                <div class="cr_center">
                <button class="button button-small aiomatic_sync_batch" id="aiomatic_sync_batch_<?php echo esc_html($aiomatic_batch->ID);?>" sync-id="<?php echo esc_html($aiomatic_batch->ID);?>"><?php echo esc_html__("Sync", 'aiomatic-automatic-ai-content-writer');?></button>
                <button class="button button-small aiomatic_view_batch" id="aiomatic_view_batch_<?php echo esc_html($aiomatic_batch->ID);?>" edit-id="<?php echo esc_html($aiomatic_batch->ID);?>"><?php echo esc_html__("Details", 'aiomatic-automatic-ai-content-writer');?></button>
<?php
if($status == 'validating' || $status == 'in_progress')
{
?>
<button class="button button-small button-link-delete aiomatic_cancel_batch" id="aiomatic_cancel_batch_<?php echo esc_html($aiomatic_batch->ID);?>" edit-id="<?php echo esc_html($aiomatic_batch->ID);?>"><?php echo esc_html__("Cancel", 'aiomatic-automatic-ai-content-writer');?></button>
<?php
}
?>
                </div>
...
<div id="mymodalfzr" class="codemodalfzr">
    <div class="codemodalfzr-content">
        ...
        <form action="#" method="post" id="aiomatic_batches_form">
            ...
            <input type="hidden" name="nonce" value="<?php echo esc_html(wp_create_nonce('aiomatic_batches'));?>">
...
<?php
    ...
    if(aiomatic_check_if_azure($aiomatic_Main_Settings))
    {
        ...
        if (!$azure_endpoint) {
            echo '<option disabled>' . esc_html__("Azure endpoint is missing in settings", 'aiomatic-automatic-ai-content-writer') . '</option>';
        } else {
            ...
            if ($response === false) {
                echo '<option disabled>' . esc_html__("Error retrieving files from Azure", 'aiomatic-automatic-ai-content-writer') . '</option>';
            } else {
                ...
                if ($http_code == 200) {
                    ...
                    if (isset($result['data']) && is_array($result['data'])) {
                        ...
                        foreach ($result['data'] as $file) {
                            if (isset($file['purpose']) && $file['purpose'] === 'batch') {
                                echo '<option value="' . esc_attr($file['id']) . '">' . esc_html($file['filename']) . ' (' . esc_html($file['id']) . ')</option>';
                                $found = true;
                            }
                        }
                        if (!$found) {
                            echo '<option disabled>' . esc_html__("No batch files available", 'aiomatic-automatic-ai-content-writer') . '</option>';
                        }
                    } else {
                        echo '<option disabled>' . esc_html__("Error processing Azure response", 'aiomatic-automatic-ai-content-writer') . '</option>';
                    }
                } else {
                    echo '<option disabled>' . esc_html__("HTTP request failed with code ", 'aiomatic-automatic-ai-content-writer') . esc_html($http_code) . '</option>';
                }
            }
        }
    }

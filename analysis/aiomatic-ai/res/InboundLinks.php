...
class AiomaticAutoInboundLinks 
{
    ...
    public function add_inbound_links($content, $max, $link_post_types, $lang, $rel_search, $current_post_id, $link_type, $link_list, $link_nofollow, $link_post_ids = '', $link_post_not_ids = '', $force_keywords = array()) 
    {
        $keywords = array();
        ...
        if(isset($aiomatic_Main_Settings['kw_method']) && $aiomatic_Main_Settings['kw_method'] == 'ai' && isset($aiomatic_Main_Settings['kw_prompt']) && $aiomatic_Main_Settings['kw_prompt'] != '')
        {
            ...
            else
            {
                ...
                $aierror = '';
                $finish_reason = '';
                $generated_text = aiomatic_generate_text($token, $kw_model, $prompt, $available_tokens, 1, 1, 0, 0, false, 'linkKeywordWriter', 0, $finish_reason, $aierror, true, false, false, '', '', 'user', $kw_assistant_id, $thread_id, '', 'disabled', '', false, false, array());
        ...
        $force_keywords = array_flip($force_keywords);
        $keywords = array_merge($keywords, $force_keywords);
        $content = $this->insert_links($content, $keywords, $max, $link_post_types, $rel_search, $current_post_id, $link_type, $link_list, $link_nofollow, $link_post_ids, $link_post_not_ids);
        return $content;
    }
    ...
    private function insert_links($content, $keywords, $max, $link_post_types, $rel_search, $post_id, $link_type, $link_list, $link_nofollow, $link_post_ids = '', $link_post_not_ids = '') 
    {
        $new_tab = ' ';
        $aiomatic_Main_Settings = get_option('aiomatic_Main_Settings', false);
        if(isset($aiomatic_Main_Settings['new_tab']) && $aiomatic_Main_Settings['new_tab'] == 'yes')
        {
        ...
        foreach ($keywords as $keyword => $score) 
        {
            ...
            if($link_type == 'internal' || $link_type == '')
            {
                ...
                {
                    $linked_posts = $this->get_linked_posts($keyword, $link_post_types, $rel_search, $doneids, $link_post_ids, $link_post_not_ids);
                    if (!empty($linked_posts)) 
                    {
                        ...
                        $link_title = $linked_posts[0]->post_title;
                        foreach ($keywords as $check_keyword => $score) 
                        {
                            if (stripos($link_title, $check_keyword) !== false && $check_keyword !== $keyword) 
                            {
                                $link_title = str_ireplace($check_keyword, '...', $link_title);
                            }
                        }
                        $replacement = sprintf(
                            '<a href="%s"' . $new_tab . 'title="%s">%s</a>',
            ...
            elseif($link_type == 'amazon')
            {
                if(strstr($content, $keyword) !== false && strlen($keyword) > 2)
                {
                    require_once(__DIR__ . '/amazon-direct.php');
                    $max_prod = 1;
                    $aiomatic_Spinner_Settings = get_option('aiomatic_Spinner_Settings', false);
                    if (isset($aiomatic_Spinner_Settings['affiliate_id']) && $aiomatic_Spinner_Settings['affiliate_id'] != '')
                    {
                    ...
                    if (isset($aiomatic_Spinner_Settings['target_country']) && $aiomatic_Spinner_Settings['target_country'] != '')
                    {
                    ...
                    $linked_posts = array();
                    $okish = true;
					$amazresult = aiomatic_amazon_get_post($keyword, trim($affiliate_id), $target_country, '', '', '', $max_prod, '1', array());
                    ...
                    if($okish == true)
                    {
                        foreach($amazresult as $amazprod)
                        {
                            $linked_posts[$amazprod->offer_title] = $amazprod->offer_url;
                        }
                    }
                    shuffle($linked_posts);
                    if (!empty($linked_posts)) 
                    {
                        reset($linked_posts); 
                        $link_title = key($linked_posts);
                        $link_link = current($linked_posts);
                        if($link_nofollow == 'on' || $link_nofollow == '1')
                        {
                            $ltemplate = '<a href="%s"' . $new_tab . 'title="%s" rel="nofollow">%s</a>';
                        }
                        else
                        {
                            $ltemplate = '<a href="%s"' . $new_tab . 'title="%s">%s</a>';
                        }
                        foreach ($keywords as $check_keyword => $score) 
                        {
                            if (stripos($link_title, $check_keyword) !== false && $check_keyword !== $keyword) 
                        ...
                        $content = preg_replace_callback(
                            ...
                            function ($matches) use (&$headings) {
                                $placeholder = "###^@!!!h3!@d1n8_" . count($headings) . "!!!@^###";
                                $headings[$placeholder] = $matches[0];
                                return $placeholder;
                            },
                            $content
                        );
                        $content = preg_replace('/\b' . preg_quote($keyword, '/') . '\b/', $replacement, $content, 1);
                        $content = str_replace(array_keys($headings), array_values($headings), $content);
                        $doneids[] = $link_link;
                        $added++;
                    }
                }
            }
            elseif($link_type == 'serp')
            {
                if(strstr($content, $keyword) !== false && strlen($keyword) > 2)
                {
                    $locale = '';
                    $linked_posts = aiomatic_internet_result_unique($keyword, false, $locale, $doneids);
                    shuffle($linked_posts);
                    if (!empty($linked_posts)) 
                    {
                        if($link_nofollow == 'on' || $link_nofollow == '1')
                        ...
                        $link_title = $linked_posts[0]['title'];
                        foreach ($keywords as $check_keyword => $score) 
                        {
                            if (stripos($link_title, $check_keyword) !== false && $check_keyword !== $keyword) 
                            {
            ...
            elseif($link_type == 'manual')
            {
                ...
                {
                    $keyword_links = array_filter($links, function($link) use ($keyword) {
                        return stripos($link, $keyword) !== false;
                    });
                    if (!empty($keyword_links)) {
                        $the_link_id = array_rand($keyword_links);
                        $my_link = $keyword_links[$the_link_id];
                    } else {
                ...
                if(strstr($content, $keyword) !== false && strlen($keyword) > 2)
                {
                    ...
                    if($link_nofollow == 'on' || $link_nofollow == '1')
                    {
                        $ltemplate = '<a href="%s"' . $new_tab . 'rel="nofollow">%s</a>';
                    }
                    else
                    {
                        $ltemplate = '<a href="%s"' . $new_tab . '>%s</a>';
                    }
                    $replacement = sprintf(
                        $ltemplate,
                        esc_url($my_link),
            ...
            elseif($link_type == 'intmanserp')
            {
                //1 manual, 2 serp, 3 internal
                $go_selector = wp_rand(1,3);
                if(count($links) == 0 && $go_selector == 1)
                {
                    $go_selector = wp_rand(2,3);
                }
                if($go_selector == 1)
                {
                    if(strstr($content, $keyword) !== false && strlen($keyword) > 2)
    ...
    private function get_linked_posts($keyword, $link_post_types, $rel_search, $post_id, $link_post_ids, $link_post_not_ids) 
    {
        global $wpdb;
        $idquery = "AND ID != %d";
        if($post_id === null)
        ...
        if (!empty($link_post_ids)) {
            $link_post_ids_array = array_map('intval', explode(',', $link_post_ids));
            $id_in_query = ' AND ID IN (' . implode(',', $link_post_ids_array) . ') ';
        }
        $id_not_in_query = '';
        if (!empty($link_post_not_ids)) {
            $link_post_not_ids_array = array_map('intval', explode(',', $link_post_not_ids));
            $id_not_in_query = ' AND ID NOT IN (' . implode(',', $link_post_not_ids_array) . ') ';
        }
        ...
        " . $idquery . "
        AND " . $like_expr . $id_in_query . $id_not_in_query . " 
        LIMIT 1";
        $sql = $wpdb->prepare(
            $mysqls,
            $argsvar

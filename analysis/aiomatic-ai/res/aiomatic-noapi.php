...
class aiomatic_no_amazon_api {
	...
	public function getItemByAsin($asin_code, $slug = '', $affiliateID = '') {
        ...
        if (stristr($exec, 'errors/validateCaptcha')) {
            throw new Exception('Amazon returned a captcha page instead of the product page, you will need to add a proxy in the plugin\'s menu.');
        }
		...
		preg_match_all ( '#data-old-hires="([^"]*?)"#', $exec, $possible_full_img );
		if (isset ( $possible_full_img [1] [0] )) 
		{
			foreach ( $possible_full_img [1] as $fimg ) 
			{
				array_unshift($item_images, $fimg);
			}
		}
		...
		elseif(stristr($exec, 'id="twister-plus-price-data-price"')){
			...
			if(isset($possible_price_matches_pre [1]))
			{
        ...
        $item_brand = '';
        if ($elements->length > 0) {
            $item_brand = trim($elements->item(0)->nodeValue);
        ...
        $ret['product_author'] = '';
        if ($elements->length > 0) {
            $item_author = trim($elements->item(0)->nodeValue);
        ...
        if ($elements->length > 0) {
            ...
            $item_summary = preg_replace('{<p data-csa-c-item-id="ai_generated_text_banner".*?</p>}s', '', $item_summary);
        ...
        if ($elements->length > 0) 
		{
            foreach ($elements as $element) 
			{
                ...
                if ($elements_review->length > 0) {
                    $reviewer_name = trim($elements_review->item(0)->nodeValue);
                }
                $elements_review = $xpath_review->query("//*[contains(@class, 'a-icon-alt')]");
                $review_rating = '';
                if ($elements_review->length > 0) {
                    $review_rating = trim($elements_review->item(0)->nodeValue);
                }
                $elements_review = $xpath_review->query("//a[@data-hook='review-title']/span[2]");
                $review_title = '';
                if ($elements_review->length > 0) {
                    $review_title = trim($elements_review->item(0)->nodeValue);
				}
				$elements_review = $xpath_review->query("//div[@data-hook='review-collapsed']/span");
				$review_text = '';
				if ($elements_review->length > 0) {
					$review_text = trim($elements_review->item(0)->nodeValue);
				}

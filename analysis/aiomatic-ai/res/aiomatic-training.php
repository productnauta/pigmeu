...
<div class="wrap">
        ...
        <div id="tab-1" class="tab-content">
         ...
         <p><?php echo esc_html__("If you are looking for a quick way to customize the AI content writer and to teach it some info about your company, I suggest you check the", 'aiomatic-automatic-ai-content-writer');?> <a href="<?php echo esc_url(admin_url('admin.php?page=aiomatic_embeddings_panel'));?>"><?php echo esc_html__("Embeddings", 'aiomatic-automatic-ai-content-writer');?></a> <?php echo esc_html__("feature of the plugin", 'aiomatic-automatic-ai-content-writer');?>.</p>
...
{
    echo '<option value="' . esc_html($bm) . '"';
    if($bm == AIOMATIC_DEFAULT_TRAINING_MODEL)
    {
        echo ' selected';
    }
    echo '>' . esc_html($bm) . '</option>';
}
?>
                </select>
            </td>
...
<form id="aiomatic_data_converter" method="post" action="">
    ...
    <table class="form-table">
        ...
        <tr>
            <th scope="row"><?php echo esc_html__("Model Base", 'aiomatic-automatic-ai-content-writer');?></th>
            <td>
            <select id="model_selector_converter_training" name="model">
<?php
foreach(AIOMATIC_TRAINING_MODELS as $bm)
{
    echo '<option value="' . esc_html($bm) . '"';
...
<table class="wp-list-table widefat fixed striped table-view-list comments">
    ...
    if($aiomatic_files && is_array($aiomatic_files) && count($aiomatic_files))
    {
        ...
        <tr>
            ...
            <td><?php echo esc_html(date('d.m.Y H:i',strtotime($aiomatic_file->post_date)));?></td>
            <td><?php echo esc_html(date('d.m.Y H:i',strtotime($aiomatic_file->post_modified)));?></td>
            <td><?php echo esc_html(size_format(filesize($file)));?></td>
            <td>
                <a class="button button-small" href="<?php echo esc_url(wp_upload_dir()['baseurl'].'/'.esc_html($aiomatic_file->post_title));?>" download>Download</a>
...
if($aiomatic_files_total > 0){
    echo esc_html__('All files', 'aiomatic-automatic-ai-content-writer') . ' (' . esc_html($aiomatic_files_total) . ')<br>';
}
?>
<table class="wp-list-table widefat fixed striped table-view-list comments">
    <thead>
...
<?php
if($aiomatic_files_total > 0){
    echo esc_html__('All fine-tunes', 'aiomatic-automatic-ai-content-writer') . ' (' . esc_html($aiomatic_files_total) . ')<br>';
}
?>
<table class="wp-list-table widefat fixed striped table-view-list comments">
    <thead>

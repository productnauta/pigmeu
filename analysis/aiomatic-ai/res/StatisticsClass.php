...
class Aiomatic_Statistics
{
        ...
        if (is_array($aiomatic_Limit_Rules)) {
            ...
            if(count($userSubs) > 1)
            {
                ...
                foreach($userSubs as $usub)
                {
                    foreach ($aiomatic_Limit_Rules as $cont => $bundle[]) {
                        ...
                        if ($paid_sub == "any" || $paid_sub == $usub){
                            $matching = true;
                        } elseif ($paid_sub == "nosub" && empty($userSubs)) {
                            $matching = true;
                        } else {
                            if ($paid_sub !== "none") {
                                $matching = false;
            ...
            else
            {
                foreach ($aiomatic_Limit_Rules as $cont => $bundle[]) {
                    ...
                    if ($paid_sub == "any" || in_array($paid_sub, $userSubs)) {
                        $matching = true;
                    } elseif ($paid_sub == "nosub" && empty($userSubs)) {
                        $matching = true;
                    } else {
    ...
    function calculatePrice(
        ...
        $openai_pricing = [
            ...
            [
                ...
                "options" => [
                    ...
                    ["option" => "1024x1792", "price" => 0.12],
                    ["option" => "1792x1024", "price" => 0.12],
                    ["option" => "1536x1024", "price" => 0.12],
                    ["option" => "1024x1536", "price" => 0.12]
                ],
            ],
            [
        ...
        elseif($model == 'stable-diffusion')
        {
            return 0;
        }
        elseif($model == 'google-image')
        {
            return 0;
        }
        else
    ...
    function getPrice($query, $answer)
    {
        ...
        if ($query->mode == "text") {
            ...
            elseif (aiomatic_is_chatgpt_45_model($model)) 
            {
                $modelBase = 'gpt45';
            }
            elseif (aiomatic_is_chatgpt_41_model($model)) 
            {
                $modelBase = 'gpt41';
            }
            elseif (aiomatic_is_chatgpt_5_model($model)) 
            {
                $modelBase = 'gpt5';
            }
            ...
            elseif(aiomatic_is_o1_pro_model($model))
            {
                $modelBase = 'o1-pro';
            }
            ...
            elseif(aiomatic_is_o3_mini_model($model))
            {
                $modelBase = 'o3-mini';
            }
            elseif(aiomatic_is_o4_mini_model($model))
            {
                $modelBase = 'o4-mini';
            }
            elseif(aiomatic_is_o3_model($model))
            {
                $modelBase = 'o3';
            }
        ...
        } elseif ($query->mode == "ideogram") {
            $modelBase = "ideogram";
            $response_units = 1;
            if (isset($query->image_size)) {
                $option = $query->image_size;
            }
        } elseif (in_array($query->model, AIOMATIC_GOOGLE_IMAGE_MODELS)) {
            $modelBase = "google-image";
            $response_units = 1;
            if (isset($query->image_size)) {
                $option = $query->image_size;
            }
    ...
    public function getDetails($query, $answer, $overrides = [])
    {
        ...
        if ($query->mode == "text" || $query->mode == "edit") {
            ...
            $type = "images";
            $units = 1;
        } elseif ($query->mode == "google-image") {
            $type = "images";
            $units = 1;
    ...
    function addCasually($query, $answer, $overrides)
    {
        ...
        if ($query->mode == "text" || $query->mode == "edit") {
            ...
            $type = "images";
            $units = 1;
        } elseif ($query->mode == "google-images") {
            $type = "images";
            $units = 1;
        } elseif ($query->mode == "text-to-speech") {
            $type = "characters";
    ...
    function calculateRealtimeCost($usage, $model = 'gpt-4o')
    {
        ...
        $textInput = $usage['input_token_details']['text_tokens'] ?? 0;
        $audioInput = $usage['input_token_details']['audio_tokens'] ?? 0;
        $cachedTextInput = $usage['input_token_details']['cached_tokens_details']['text_tokens'] ?? 0;
        $cachedAudioInput = $usage['input_token_details']['cached_tokens_details']['audio_tokens'] ?? 0;
        $textOutput = $usage['output_token_details']['text_tokens'] ?? 0;
        $audioOutput = $usage['output_token_details']['audio_tokens'] ?? 0;

        $nonCachedTextInputTokens = max($textInput - $cachedTextInput, 0);
        $nonCachedAudioInputTokens = max($audioInput - $cachedAudioInput, 0);

        $textInputCost       = ($nonCachedTextInputTokens / 1_000_000) * $realtime_pricing[$model]['text_input'];
        $textCachedInputCost = ($cachedTextInput / 1_000_000) * $realtime_pricing[$model]['text_cached_input'];

        $audioInputCost       = ($nonCachedAudioInputTokens / 1_000_000) * $realtime_pricing[$model]['audio_input'];
        $audioCachedInputCost = ($cachedAudioInput / 1_000_000) * $realtime_pricing[$model]['audio_cached_input'];
        
        $textOutputCost  = ($textOutput / 1_000_000) * $realtime_pricing[$model]['text_output'];
        $audioOutputCost = ($audioOutput / 1_000_000) * $realtime_pricing[$model]['audio_output'];

        $totalCost = $textInputCost
                   + $textCachedInputCost
                   + $audioInputCost
                   + $audioCachedInputCost
                   + $textOutputCost
                   + $audioOutputCost;

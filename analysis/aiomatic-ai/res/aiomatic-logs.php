...
<div>
<div class="wrap gs_popuptype_holder seo_pops">
   <h2 class="cr_center"><?php echo esc_html__("Logs & Translations", 'aiomatic-automatic-ai-content-writer');?></h2>
<nav class="nav-tab-wrapper">
        ...
        <a href="#tab-3" class="nav-tab"><?php echo esc_html__("Language Pack Manager", 'aiomatic-automatic-ai-content-writer');?></a>
    </nav>
...
<table class="widefat" cellspacing="0">
	...
	<?php if ( function_exists( 'ini_get' ) ) : ?>
        <tr>
            ...
            <td><?php echo esc_html(size_format( aiomatic_let_to_num( ini_get( 'post_max_size' ) ) )); ?></td>
        </tr>
        <tr>
            ...
            <td>
				<?php
					...
					if ( 600 > $time_limit && 0 != $time_limit ) {
						echo '<mark class="error">' . sprintf( wp_kses_post( __( '%1$s - We recommend setting max execution time to at least 600. <br /> To generate long articles, <strong>1000</strong> seconds of max execution time is recommended.<br />See: <a href="%2$s" target="_blank" rel="noopener noreferrer">Increasing max execution to PHP</a>', 'aiomatic-automatic-ai-content-writer' ) ), esc_html($time_limit), 'https://coderevolution.ro/knowledge-base/faq/how-to-increase-the-max_execution_time-settings-on-your-server/' ) . '</mark>';
					} else {
						echo '<mark class="yes">' . esc_html( $time_limit ) . '</mark>';
						if ( 1000 > $time_limit && 0 != $time_limit ) {
							echo '<br /><mark class="error">' . sprintf( wp_kses_post( __( '%1$s - We recommend setting max execution time to at least 600. <br /> To generate long articles, <strong>1000</strong> seconds of max execution time is recommended.<br />See: <a href="%2$s" target="_blank" rel="noopener noreferrer">Increasing max execution to PHP</a>', 'aiomatic-automatic-ai-content-writer' ) ), esc_html($time_limit), 'https://coderevolution.ro/knowledge-base/faq/how-to-increase-the-max_execution_time-settings-on-your-server/' ) . '</mark>';
						}
					}
				?>
            </td>
            ...
            <td>
				<?php
					...
					if ( $max_input_vars < $required_input_vars ) {
						echo '<mark class="error">' . sprintf( wp_kses_post( __( '%1$s - Recommended Value: %2$s.<br />Max input vars limitation will truncate POST data such as menus. See: <a href="%3$s" target="_blank" rel="noopener noreferrer">Increasing max input vars limit.</a>', 'aiomatic-automatic-ai-content-writer' ) ), esc_html($max_input_vars), '<strong>' . esc_html($required_input_vars) . '</strong>', 'http://sevenspark.com/docs/ubermenu-3/faqs/menu-item-limit' ) . '</mark>';
					} else {
            ...
            <tr>
                ...
                <td>
					<?php
						...
						$required_input_vars = $required_input_vars + ( 500 + 1000 );
						if ( $max_input_vars < $required_input_vars ) {
							echo '<mark class="error">' . sprintf( wp_kses_post( __( '%1$s - Recommended Value: %2$s.<br />Max input vars limitation will truncate POST data such as menus. See: <a href="%3$s" target="_blank" rel="noopener noreferrer">Increasing max input vars limit.</a>', 'aiomatic-automatic-ai-content-writer' ) ), esc_html($max_input_vars), '<strong>' . esc_html($required_input_vars + ( 500 + 1000 )) . '</strong>', 'http://sevenspark.com/docs/ubermenu-3/faqs/menu-item-limit' ) . '</mark>';
						} else {
							echo '<mark class="yes">' . esc_html( $max_input_vars ) . '</mark>';
						}
					?>
            ...
            <tr>
                ...
                <td><?php
						...
						if ( $suhosin_max_value_length < $recommended_max_value_length ) {
							echo '<mark class="error">' . sprintf( wp_kses_post( __( '%1$s - Recommended Value: %2$s.<br />Post Max Value Length limitation may prohibit the Theme Options data from being saved to your database. See: <a href="%3$s" target="_blank" rel="noopener noreferrer">Suhosin Configuration Info</a>.', 'aiomatic-automatic-ai-content-writer' ) ), esc_html($suhosin_max_value_length), '<strong>' . esc_html($recommended_max_value_length) . '</strong>', 'http://suhosin.org/stories/configuration.html' ) . '</mark>';
						} else {
...
?>
	...
	<table class="wp-list-table widefat fixed" cellspacing="0">
		<thead>
		<tr>
			<th colspan="2"><h4><?php esc_html_e( 'System Information', 'aiomatic-automatic-ai-content-writer' ) ?></h4></th>
		</tr>
		<tr>
			<th width="35%"><?php esc_html_e( 'Setting', 'aiomatic-automatic-ai-content-writer' ) ?></th>
			<th><?php esc_html_e( 'Value', 'aiomatic-automatic-ai-content-writer' ) ?></th>
		</tr>
		</thead>
		<tbody>
		...
		foreach ( $active_plugins as $plugin ) {
			...
			if ( ! empty( $plugin_data['Name'] ) ) {
                ...
                <tr>
                    ...
                    <td class="help">&nbsp;</td>
                    <td><?php echo esc_html__( 'by', 'aiomatic-automatic-ai-content-writer' ) . '&nbsp;' . esc_html($plugin_data['Author']) . ' &ndash; ' . esc_html( $plugin_data['Version'] ) ; ?></td>
                </tr>
				<?php
			}
		}
...
$local_language_dir = WP_PLUGIN_DIR . '/aiomatic-automatic-ai-content-writer/languages';
if (!$wp_filesystem->exists($local_language_dir)) {
    wp_mkdir_p($local_language_dir);
}
$available_packs = aiomatic_fetch_available_language_packs($language_pack_url);
$installed_packs = aiomatic_get_installed_language_packs($local_language_dir);
?>
<h2><?php esc_html_e('Language Pack Manager', 'aiomatic-automatic-ai-content-writer'); ?></h2>
<p><?php esc_html_e('Manage the language packs for the Aiomatic plugin. Below is the list of available language packs, their status, and download links. You can also contact us if you want to contribute translations in your own language.', 'aiomatic-automatic-ai-content-writer'); ?></p>
<table class="wp-list-table widefat fixed striped">
    <thead>
        <tr>
            <th><?php esc_html_e('Language', 'aiomatic-automatic-ai-content-writer'); ?></th>
            <th><?php esc_html_e('Status', 'aiomatic-automatic-ai-content-writer'); ?></th>
            <th><?php esc_html_e('Direct Download', 'aiomatic-automatic-ai-content-writer'); ?></th>
            <th><?php esc_html_e('Action', 'aiomatic-automatic-ai-content-writer'); ?></th>
        </tr>
    </thead>
    <tbody>
    <?php if(is_array($available_packs)){
        foreach ($available_packs as $pack): ?>
        <tr>
            <td><?php echo esc_html($pack); ?></td>
            <td>
                <?php
                $installed_packs = aiomatic_get_installed_language_packs_meta();
                if(in_array($pack, $installed_packs))
                {
                    echo esc_html__('Installed', 'aiomatic-automatic-ai-content-writer');
                    $local_language_dir = plugin_dir_path(dirname(__FILE__)) . 'languages';;
                    $mo_path = $local_language_dir . '/' . $pack . '.mo';
                    $po_path = $local_language_dir . '/' . $pack . '.po';
                    if (!$wp_filesystem->exists($mo_path) || !$wp_filesystem->exists($po_path)) {
                        echo ' - ' . esc_html__('Translation files are missing', 'aiomatic-automatic-ai-content-writer');
                    }
                }
                else
                {
                    echo esc_html__('Not Installed', 'aiomatic-automatic-ai-content-writer');
                }
                ?>
            </td>
            <td>
                <a href="<?php echo esc_url($language_pack_url . $pack . '.mo'); ?>" download="<?php echo esc_url($language_pack_url . $pack . '.mo'); ?>" target="_blank">
                    <?php esc_html_e('Download .mo', 'aiomatic-automatic-ai-content-writer'); ?>
                </a>
                |
                <a href="<?php echo esc_url($language_pack_url . $pack . '.po'); ?>" target="_blank">
                    <?php esc_html_e('View .po', 'aiomatic-automatic-ai-content-writer'); ?>
                </a>
            </td>
            <td>
                <?php if (!in_array($pack, $installed_packs)): ?>
                    <button class="button aiomatic-language-action" data-action="install_language" data-pack="<?php echo esc_attr($pack); ?>">
                        <?php esc_html_e('Install', 'aiomatic-automatic-ai-content-writer'); ?>
                    </button>
                <?php else: ?>
                    <button class="button aiomatic-language-action" data-action="uninstall_language" data-pack="<?php echo esc_attr($pack); ?>">
                        <?php esc_html_e('Uninstall', 'aiomatic-automatic-ai-content-writer'); ?>
                    </button>
                <?php endif; ?>
            </td>
        </tr>
...
<hr class="cr_special_hr"/>
    <h3><?php esc_html_e('Want to Help Translate Aiomatic?', 'aiomatic-automatic-ai-content-writer'); ?></h3>
    <p>
        <?php esc_html_e('If you would like to contribute translations for Aiomatic in your language, I would love your help!', 'aiomatic-automatic-ai-content-writer'); ?>
        <br/>
        <?php esc_html_e('Contact me at:', 'aiomatic-automatic-ai-content-writer'); ?>
        <a href="mailto:support@coderevolution.ro">support@coderevolution.ro</a>
    </p>
    <p><?php esc_html_e('I appreciate your support in making Aiomatic accessible to users worldwide.', 'aiomatic-automatic-ai-content-writer'); ?></p><br/><h2 class="aiomatic-inner-title"><?php echo esc_html__('Tutorial Video:', 'aiomatic-automatic-ai-content-writer');?></h2>
<?php if(!isset($aiomatic_Main_Settings['hide_videos']) || trim($aiomatic_Main_Settings['hide_videos']) != 'on'){
?>
    <div class="cr_center embedtool"><iframe src="https://www.youtube.com/embed/wt18jwFTyg4" allowfullscreen></iframe></div>
<?php } ?>
</div>
<div id="tab-1" class="tab-content">
      ...
      <div>
         ...
         <input name="aiomatic_nonce" type="hidden" value="<?php echo esc_html(wp_create_nonce('openai-secret-nonce'));?>"></form>
         </div>
         <br/>
         <hr class="cr_special_hr"/>
         <div>
   ...
   <div id="tab-0" class="tab-content">
         ...
         <h3>
            ...
            <div class="bws_help_box bws_help_box_right dashicons dashicons-editor-help cr_align_middle">
               <div class="bws_hidden_help_text cr_min_260px">
                  <?php
                     echo esc_html__('This is the main log of your plugin. Here will be listed every single instance of the rules you run or are automatically run by schedule jobs (if you enable logging, in the plugin configuration). Log location: ', 'aiomatic-automatic-ai-content-writer') . esc_html(trailingslashit(get_temp_dir()) . 'aiomatic_info.log');
                     ?>
               </div>
            </div>
         </h3>
      ...
      <hr/>
         <div>
            <?php
               if($wp_filesystem->exists(trailingslashit(get_temp_dir()) . 'aiomatic_info.log'))
               {
                    $log = $wp_filesystem->get_contents(trailingslashit(get_temp_dir()) . 'aiomatic_info.log');
                    $log = esc_html($log);$log = str_replace('&lt;br/&gt;', '<br/>', $log);echo wp_kses_post($log);
               }
               else
               {

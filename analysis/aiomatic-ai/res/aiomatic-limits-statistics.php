...
function aiomatic_save_restrictions($data) {
   ...
   for($i = 0; $i < sizeof($data['user_credits']); ++$i) {
         ...
         $bundle[] = trim( sanitize_text_field( $data['paid_sub'][$i] ) );
...
<div>
  <div class="bws_help_box bws_help_box_right dashicons dashicons-editor-help cr_align_middle">
                           <div class="bws_hidden_help_text" >' . esc_html__('Integration with \'Paid Memberships Pro\'', 'aiomatic-automatic-ai-content-writer') . '
         </div>
      </div>
      <b><a href="https://www.paidmembershipspro.com/pricing/" target="_blank">' . esc_html__("Paid Memberships Pro", 'aiomatic-automatic-ai-content-writer')  . '</a>&nbsp;' . esc_html__("Subscription Plan:", 'aiomatic-automatic-ai-content-writer') . '</b>
      
      </td><td>
      <select name="aiomatic_Limit_Rules[paid_sub][]" class="cr_width_full">
      <option value="none" ';
         if($paid_sub === 'none')
         {
               $output .= 'selected="selected"';
   ...
   if (!empty($limitations)) {
      ...
      $output .= '</select></td>
            ...
            <select required multiple name="aiomatic_Menu_Rules[menu_limit' . esc_html($cont) . '][]" class="cr_width_full">
                ...
                <option value="aiomatic_realtime_chatbot_panel" ';
                if(in_array('aiomatic_realtime_chatbot_panel', $user_menus))
                {
                    $output .= 'selected="selected"';
                }
                $output .= '>' . esc_html__('AI Realtime Chat', 'aiomatic-automatic-ai-content-writer') . '</option>
                ...
                <option value="aiomatic_mcp" ';
                if(in_array('aiomatic_mcp', $user_menus))
                {
                    $output .= 'selected="selected"';
                }
                $output .= '>' . esc_html__('AI Site Builder MCP Server', 'aiomatic-automatic-ai-content-writer') . '</option>
                ...
                $output .= '>' . esc_html__('Logs & Translations', 'aiomatic-automatic-ai-content-writer') . '</option>';
                $output .= '</select></td>
              <td class="cr_30 cr_center" ><span class="wpaiomatic-delete">X</span></td>
         </tr>	
         ';
...
function aiomatic_openai_status()
{
  ...
  if (isset($aiomatic_Limit_Settings['block_userips'])) {
      $block_userips = $aiomatic_Limit_Settings['block_userips'];
  } else {
      $block_userips = '';
  }
...
<div class="wrap">
        <nav class="nav-tab-wrapper">
            ...
            <a href="#tab-x" datahref="<?php echo esc_url(admin_url('admin.php?page=aiomatic_admin_settings#tab-5'));?>" class="nav-tab"><?php echo esc_html__("Limits & Statistics Settings", 'aiomatic-automatic-ai-content-writer');?></a>
        </nav>
        ...
        <div id="tab-1" class="tab-content">
         ...
         if (isset($aiomatic_Main_Settings['enable_tracking']) && $aiomatic_Main_Settings['enable_tracking'] === 'on') {
            ...
            else
            {
               ...
               echo '<tr><th><a class="aiomatic_normal" href="' . esc_url($id_sort) . '">' . esc_html__("ID", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('id');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($user_sort) . '">' . esc_html__("User", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('user');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($ip_sort) . '">' . esc_html__("IP", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('ip');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($source_sort) . '">' . esc_html__("Source", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('source');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($model_sort) . '">' . esc_html__("Model", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('model');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($assistant_id_sort) . '">' . esc_html__("Assistant ID", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('assistant_id');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($mode_sort) . '">' . esc_html__("Mode", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('mode');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($units_sort) . '">' . esc_html__("Units", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('units');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($type_sort) . '">' . esc_html__("Type", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('type');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($price_sort) . '">' . esc_html__("Price", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('price');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($time_sort) . '">' . esc_html__("Time", 'aiomatic-automatic-ai-content-writer');
               aiomatic_display_arrows('time');
               echo '</a></th><th><a class="aiomatic_normal" href="' . esc_url($session_sort) . '">' . esc_html__("Session ID", 'aiomatic-automatic-ai-content-writer');
               ...
               foreach($statistics_res['rows'] as $stat_row)
               {
                  ...
                  echo '<tr>';
                  echo '<td>' . esc_html($stat_row['id']) . '</td>';
                  if(isset($myusers[$stat_row['userId']]))
                  {
                     echo '<td>' . esc_html($myusers[$stat_row['userId']]) . '</td>';
                  }
                  else
                  {
                     echo '<td>ID: ' . esc_html($stat_row['userId']) . '</td>';
                  }
                  echo '<td>' . esc_html($stat_row['ip']) . '</td>';
                  echo '<td>' . esc_html($stat_row['env']) . '</td>';
                  echo '<td>' . esc_html($stat_row['model']) . '</td>';
                  if(isset($stat_row['assistant_id']) && !empty($stat_row['assistant_id']))
                  {
                     echo '<td>' . esc_html($stat_row['assistant_id']) . '</td>';
                  }
                  else
                  {
                     echo '<td>-</td>';
                  }
                  echo '<td>' . esc_html($stat_row['mode']) . '</td>';
                  echo '<td>' . esc_html($stat_row['units']) . '</td>';
                  echo '<td>' . esc_html($stat_row['type']) . '</td>';
                  ...
                  {
                     echo '<td>' . esc_html($stat_row['price']) . '$</td>';
                  }
                  echo '<td>' . esc_html($stat_row['time']) . '</td>';
                  echo '<td>' . esc_html($stat_row['session']) . '</td>';
                  echo '</tr>';
               }
               echo '</table>';
               echo '</div>';
...
$roles = aiomatic_get_editable_roles();
   ...
   <div class="cr_autocomplete">
      <input type="password" id="PreventChromeAutocomplete2" 
            ...
            <td>
               <div>
                  <div class="bws_help_box bws_help_box_right dashicons dashicons-editor-help cr_align_middle">
                     <div class="bws_hidden_help_text cr_min_260px">
                        <?php
                           echo esc_html__("Do you want to set a comma separated list of user IPs which are blocked from using the AI?", 'aiomatic-automatic-ai-content-writer');
                           ?>
                     </div>
                  </div>
                  <b><?php echo esc_html__("Restrict User IPs List From Using The AI:", 'aiomatic-automatic-ai-content-writer');?></b>
            </td>
            <td>
            <input type="text" name="aiomatic_Limit_Settings[block_userips]" id="block_userips" placeholder="User IPs to block" value="<?php echo esc_attr($block_userips);?>" class="cr_width_full"/>
            </td>
         </tr>
        <tr>
                     <td>
         ...
         <tr class="hideLimits">
            ...
            <td>
               ...
               <input type="number" id="user_credits" step="0.01" min="0" placeholder="<?php echo esc_html__("Maximum Credits For Users", 'aiomatic-automatic-ai-content-writer');?>" name="aiomatic_Limit_Settings[user_credits]" value="<?php
                     ...
                     ?>"/>
                     <select id="user_credit_type_settings" name="aiomatic_Limit_Settings[user_credit_type]" >
                     <option value="queries"<?php
                        if ($user_credit_type == "queries") {
                              echo " selected";
                        }
...
{
?>
   <option value="<?php echo esc_html($urole);?>"><?php echo esc_html($urole);?></option>
<?php
}
?>
                                       </select>
                                    ...
                                    <tr>
                                       <td class="cr_min_width_200">
                                             <div class="bws_help_box bws_help_box_right dashicons dashicons-editor-help cr_align_middle">
                                                <div class="bws_hidden_help_text cr_min_260px">
                                                   <?php
                                                      echo esc_html__("Integration with 'Paid Memberships Pro'", 'aiomatic-automatic-ai-content-writer');
                                                      ?>
                                                </div>
                                             </div>
                                             <b><a href="https://www.paidmembershipspro.com/pricing/" target="_blank"><?php echo esc_html__("Paid Memberships Pro", 'aiomatic-automatic-ai-content-writer');?></a>&nbsp;<?php echo esc_html__("Subscription Plan:", 'aiomatic-automatic-ai-content-writer');?></b>
                                       </td>
                                       <td>
                                       <select name="aiomatic_Limit_Rules[paid_sub][]" class="cr_width_full">
                                       <option value="none" selected><?php echo esc_html__("Don't check", 'aiomatic-automatic-ai-content-writer');?></option>
...
foreach($roles as $urole => $caps)
{
   ...
   echo '<option value="'; echo esc_html($urole); echo '"'; if(in_array($urole, $additional_roles)){ echo ' selected';} echo '>'; echo esc_html($urole); echo '</option>';
}
?>
                  </select>
               </div>
        ...
        <h3><?php echo esc_html__("User Role Based Plugin Menu Restrictions:", 'aiomatic-automatic-ai-content-writer');?></h3>
         ...
         ?>
   <div class="cr_autocomplete">
      <input type="password" id="PreventChromeAutocomplete1" 
         name="PreventChromeAutocomplete" autocomplete="address-level4" />
   </div>
         <table class="responsive table cr_main_table">
            <thead>
            ...
            <tbody>
               ...
               <tr>
                  <td class="cr_td_xo"><input type="text" name="aiomatic_Menu_Rules[rule_description][]" id="rule_description_menu" class="cr_center" placeholder="Rule ID" value="" class="cr_width_full"/></td>
                  <td class="cr_custx">
...
?>
               ...
               <td class="cr_custx">
                  <select id="menu_limit" multiple name="aiomatic_Menu_Rules[menu_limit][]" >
                     ...
                     <option value="aiomatic_realtime_chatbot_panel"><?php echo esc_html__("AI Realtime Chat", 'aiomatic-automatic-ai-content-writer');?></option>
                     ...
                     <option value="aiomatic_mcp"><?php echo esc_html__("AI Site Builder MCP Server", 'aiomatic-automatic-ai-content-writer');?></option>
                     ...
                     <option value="aiomatic_logs"><?php echo esc_html__("Logs & Translations", 'aiomatic-automatic-ai-content-writer');?></option>
                  </select></td>
                  <td class="cr_30 cr_center" ><span class="cr_30">X</span></td>
                     </tr>
                     </tbody>
...
echo '&nbsp;<a href="' . esc_url(add_query_arg( array( 'older' => $older + 12 ), $current_page )) . '"><svg xmlns="http://www.w3.org/2000/svg" focusable="false" class="aiomatic-paging-controller-icon disabled" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" style="transform: rotate(360deg);"><path fill="currentColor" d="M18.41 7.41L17 6l-6 6l6 6l1.41-1.41L13.83 12l4.58-4.59m-6 0L11 6l-6 6l6 6l1.41-1.41L7.83 12l4.58-4.59Z"></path></svg></a>';
echo '&nbsp;<a href="' . esc_url(add_query_arg( array( 'older' => $older + 1 ), $current_page )) . '"><svg xmlns="http://www.w3.org/2000/svg" focusable="false" class="aiomatic-paging-controller-icon disabled" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" style="transform: rotate(360deg);"><path fill="currentColor" d="M15.41 16.58L10.83 12l4.58-4.59L14 6l-6 6l6 6l1.41-1.42Z"></path></svg></a>';
echo '&nbsp;<span class="aiomatic-paging">' . esc_html__("Page", 'aiomatic-automatic-ai-content-writer') . '&nbsp;' . ($older + 1) . '&nbsp;</span>';
if($older > 0)
{
   echo '&nbsp;<a href="' . esc_url(add_query_arg( array( 'older' => $older - 1 ), $current_page )) . '"><svg xmlns="http://www.w3.org/2000/svg" focusable="false" class="aiomatic-paging-controller-icon disabled" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" style="transform: rotate(360deg);"><path fill="currentColor" d="M8.59 16.58L13.17 12L8.59 7.41L10 6l6 6l-6 6l-1.41-1.42Z"></path></svg></a>';
   echo '&nbsp;<a href="' . esc_url(add_query_arg( array( 'older' => 0 ), $current_page )) . '"><svg xmlns="http://www.w3.org/2000/svg" focusable="false" class="aiomatic-paging-controller-icon disabled" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" style="transform: rotate(360deg);"><path fill="currentColor" d="M5.59 7.41L7 6l6 6l-6 6l-1.41-1.41L10.17 12L5.59 7.41m6 0L13 6l6 6l-6 6l-1.41-1.41L16.17 12l-4.58-4.59Z"></path></svg></a>';
}
echo '</span>';

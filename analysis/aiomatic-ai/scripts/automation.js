...
function aiomatic_compareIds(id1, id2) {
    ...
    return id1.localeCompare(id2, 'en', { numeric: true });
}
...
function updateSortableInputAI(i, suff)
{
    ...
    jQuery('#aiomatic_sortable_cards' + suff + i + ' > li').each(function(index) 
    {
        ...
        else if (cardType === 'dalle_ai_image' || cardType === 'stable_ai_image' || cardType === 'google_ai_image') 
        {
            var prompt = li.find('.prompt').val();
            var model = li.find('.model').val();
        ...
        else if (cardType === 'ideogram_ai_image') 
        {
            var prompt = li.find('.prompt').val();
            var image_size = li.find('.image_size').val();
            parameters = {
        ...
        else if (cardType === 'stt_openai') 
        {
            var audio_url = li.find('.audio_url').val();
            var prompt = li.find('.prompt').val();
            var purpose = li.find('.purpose').val();
            var model = li.find('.model').val();
            var language = li.find('.language').val();
            var temperature = li.find('.temperature').val();
            parameters = {
                'audio_url': audio_url,
                'prompt': prompt,
                'purpose': purpose,
        ...
        else if (cardType === 'ai_forms_omniblock') 
        {
            var api_key = li.find('.api_key').val();
            parameters = {
                'api_key': api_key,
        ...
        else if (cardType === 'ai_chatbot_omniblock') 
        {
            parameters = {
                'critical': criticalBlock,
                'disabled': disabledBlock
        ...
        else if (cardType === 'wp_event_trigger') 
        {
            var wp_event = li.find('.wp_event').val();
            parameters = {
                'wp_event': wp_event,
                'critical': criticalBlock,
                'disabled': disabledBlock
        ...
        else if (cardType === 'wait_block') 
        {
            var wait = li.find('.wait').val();
            parameters = {
                'wait': wait,
                'critical': criticalBlock,
                'disabled': disabledBlock
        ...
        var setNewId = aiomatic_increment(indentificator);
        jQuery('#aiomatic_new_card_types' + i + ' > li').each(function(index) 
        {
            var xli = jQuery(this);
            var cainput = xli.find('input[class*="omniblock-id"]');
            var currentVal = cainput.val();
            if (aiomatic_compareIds(setNewId, currentVal) > 0) {
                cainput.val(setNewId);
                var shortcodeinput = xli.find('input[class*="omniblock-shortcodes"]');
                var shortcodeVal = shortcodeinput.val();
                if (shortcodeVal) {
                    shortcodeVal = shortcodeVal.replace(
                        new RegExp('_' + currentVal + '%%', 'g'),
                        '_' + setNewId + '%%'
                    );
                    shortcodeinput.val(shortcodeVal);
                }
            }
        });
        jQuery('#aiomatic_new_card_types_edit > li').each(function(index) 
        {
            var xli = jQuery(this);
            var cainput = xli.find('input[class*="omniblock-id"]');
            var currentVal = cainput.val();
                ...
                if(bl_id !== undefined)
                {
                    ...
                    if(bl_code !== undefined)
                    {
                        ...
                        if(bltype !== null)
                        {
                            ...
                            else if(bltype == 'google_ai_image')
                            {
                                bl_code.val('%%google_image_' + indentificator + '%%');
                            }
                            ...
                            else if(bltype == 'ideogram_ai_image')
                            {
                                bl_code.val('%%ideogram_image_' + indentificator + '%%');
                            }
                            ...
                            else if(bltype == 'stt_openai')
                            {
                                bl_code.val('%%audio_transcript_' + indentificator + '%%');
                            }
                            ...
                            else if(bltype == 'ai_forms_omniblock')
                            {
                                bl_code.val('%%ai_forms_full_input_' + indentificator + '%%');
                            }
                            else if(bltype == 'ai_chatbot_omniblock')
                            {
                                bl_code.val('%%ai_chatbot_full_input_' + indentificator + '%%');
                            }
                            else if(bltype == 'wp_event_trigger')
                            {
                                bl_code.val('%%event_param1_' + indentificator + '%%,%%event_param2_' + indentificator + '%%,%%event_param3_' + indentificator + '%%,%%event_param4_' + indentificator + '%%,%%event_param5_' + indentificator + '%%,%%event_param6_' + indentificator + '%%,%%event_param7_' + indentificator + '%%,%%event_param8_' + indentificator + '%%,%%event_param9_' + indentificator + '%%,%%event_param10_' + indentificator + '%%');
                            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(Array.isArray(rez))
                    {
                        ...
                        if(isEmpty === true)
                        {
                            alert('Failed to run OmniBlock, please check the \'Logs & Translations\' menu for details!');
                        }
...
jQuery(document).ready(function($) 
{
    ...
    var codemodalfzr_ai = document.getElementById('mymodalfzr_ai');
    var btn_backup = document.getElementById("aiomatic_backup_templates");
    var ai_backup = document.getElementById("aiomatic_ai_templates");
    var span_backup = document.getElementById("aiomatic_close_backup");
    var span_ai = document.getElementById("aiomatic_close_ai");
    if(btn_backup != null)
    {
    ...
    if(ai_backup != null)
    {
        ai_backup.onclick = function(e) {
            e.preventDefault();
            codemodalfzr_ai.style.display = "block";
        }
    }
    if(span_backup != null)
    {
    ...
    if(span_ai != null)
    {
        span_ai.onclick = function() {
            codemodalfzr_ai.style.display = "none";
        }
    }
    ...
    var aiomatic_omni_ai_button = jQuery('#aiomatic_omni_ai_button');
    aiomatic_omni_ai_button.on('click', function (e){
        e.preventDefault();
        if(confirm('Are you sure you want to generate an OmniBlock Templates using this AI prompt and model?'))
        {
            unsaved = false;
            var aiomatic_ai_progress = jQuery('.aiomatic_ai_progress');
            var aiomatic_error_message = jQuery('.aiomatic-ai-error-msg');
            var aiomatic_ai_success = jQuery('.aiomatic_ai_success');
            var ai_model = jQuery('.aiomatic_omni_model').val();
            var ai_prompt = jQuery('#aiomatic_omni_prompt').val();
            var aiomatic_omni_name = jQuery('#aiomatic_omni_name').val();
            if(jQuery("#aiomatic_overwrite").is(':checked'))
            {
                var overwrite = '1';
            }
            ...
            if(aiomatic_omni_name == '')
            {
                alert('Please fill in the AI template name settings field!');
                return;
            }
            if(ai_prompt == '' || ai_model == '')
            {
                alert('Please fill in the AI prompt and select a model!');
                return;
            }
            var formData = new FormData();
            formData.append('action', 'aiomatic_omni_ai');
            formData.append('nonce', aiomatic_object.nonce);
            formData.append('ai_prompt', ai_prompt);
            formData.append('ai_model', ai_model);
            formData.append('overwrite', overwrite);
            formData.append('omni_name', aiomatic_omni_name);
            jQuery.ajax({
                url: aiomatic_object.ajax_url,
                type: 'POST',
                dataType: 'JSON',
                data: formData,
                beforeSend: function (){
                    aiomatic_ai_progress.find('span').css('width','0');
                    aiomatic_ai_progress.show();
                    aiomaticLoading2(aiomatic_omni_ai_button);
                    aiomatic_error_message.hide();
                    aiomatic_ai_success.hide();
                },
                xhr: function() {
                    var xhr = jQuery.ajaxSettings.xhr();
                    xhr.upload.addEventListener("progress", function(evt) {
                        if (evt.lengthComputable) {
                            var percentComplete = evt.loaded / evt.total;
                            aiomatic_ai_progress.find('span').css('width',(Math.round(percentComplete * 100))+'%');
                        }
                    }, false);
                    return xhr;
                },
                success: function(res) {
                    if(res.status === 'success'){
                        aiomaticRmLoading(aiomatic_omni_ai_button);
                        aiomatic_ai_progress.hide();
                        aiomatic_ai_success.show();
                        location.reload();
                    }
                    else{
                        aiomaticRmLoading(aiomatic_omni_ai_button);
                        aiomatic_ai_progress.find('small').html('Error');
                        aiomatic_ai_progress.addClass('aiomatic_error');
                        aiomatic_error_message.html(res.msg);
                        aiomatic_error_message.show();
                    }
                },
                ...
                error: function (r, s, error){
                    ...
                    aiomatic_ai_progress.addClass('aiomatic_error');
                    aiomatic_ai_progress.find('small').html('Error');
                    alert('Error in processing AI OmniBlock template generator: ' + error);
                    aiomatic_error_message.show();
                }
            });
        }

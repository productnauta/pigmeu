"use strict";
function hasRedBorder() 
{
    const textarea = document.getElementById('aiomatic_bot_workflow');
    if (!textarea) {
        console.error('Textarea with ID "aiomatic_bot_workflow" not found.');
        return false;
    }
...
document.addEventListener('DOMContentLoaded', function () {
    const downloadButton = document.getElementById('aidownloadTextareaContent');
    const loadButton = document.getElementById('ailoadTextareaContent');
...
function assistantChanged()
{
    ...
    if(selectedAssist != '' && selectedAssist != null)
    {
        jQuery('.hideAssistantIDMain').attr('disabled','disabled');
    }
    else
    {
        jQuery('.hideAssistantIDMain').removeAttr('disabled');
    }
}
        ...
        jQuery.ajax({
            ...
            success: function (res){
                if(res.status === 'success')
                {
                    ...
                    {
                        alert('Failed to decode message ' + res.msg);
                    }
                }
                else
                {
...
function multiChatChanged_b()
{
    var selectedValues = jQuery('#multibot_b').val();
    if (selectedValues && selectedValues.length > 0) 
    {
        var formattedValues = selectedValues.map(value => value.replace(/"/g, "'")).join(',');
        var customized_chatbot = jQuery('#customized_chatbot').text();
        if (customized_chatbot !== undefined) {
            customized_chatbot = customized_chatbot.replace(
                /additional_bots="([^"]*?)"/g,
                'additional_bots="' + formattedValues + '"'
            );
            jQuery('#customized_chatbot').text(customized_chatbot);
        }
    } 
...
function conversationStartersChanged_b()
{
    var selected = jQuery('#conversation_starters_b').val();
    if(selected != undefined)
    {
        selected = selected.replace(/"/g, "'");
        var customized_chatbot = jQuery('#customized_chatbot').text();
        if(customized_chatbot !== undefined)
        {
            customized_chatbot = customized_chatbot.replace(/conversation_starters="([^"]*?)"/g, 'conversation_starters="' + selected.replace('"', "'") + '"');
            jQuery('#customized_chatbot').text(customized_chatbot);
        }
    }
}
...
function botWorkflowChanged_b()
{
    var selected = jQuery('#bot_workflow_b').val();
    var customized_chatbot = jQuery('#customized_chatbot').text();
    if(customized_chatbot !== undefined)
    {
        if(selected != undefined)
        {
            try 
            {
                selected = JSON.stringify(JSON.parse(selected));
                if(selected == '[]')
                {
                    selected = '';
                }
                const base64Encoded = btoa(selected);
                customized_chatbot = customized_chatbot.replace(/bot_workflow="([^"]*?)"/g, 'bot_workflow="' + base64Encoded + '"');
                jQuery('#customized_chatbot').text(customized_chatbot);
            }
            catch(error) 
            {
                console.log('Error in JSON parsing ' + error);
                customized_chatbot = customized_chatbot.replace(/bot_workflow="([^"]*?)"/g, 'bot_workflow=""');
                jQuery('#customized_chatbot').text(customized_chatbot);
            }
        }
    }
...
function submitImageChanged_b()
{
    var selected = jQuery('#submit_image_b').val();
    var text_input = jQuery('.aichatsubmitbut');
    if(selected != undefined && text_input !== null)
    {
        var customized_chatbot = jQuery('#customized_chatbot').text();
        if(customized_chatbot !== undefined)
        {
            customized_chatbot = customized_chatbot.replace(/submit_image="([^"]*?)"/g, 'submit_image="' + selected.replace('"', "'") + '"');
            jQuery('#customized_chatbot').text(customized_chatbot);
        }
        if(selected != '')
        {
            text_input.html('<img src="' + selected.replace('"', "'") + '" />');
        }
    }
}
function submitFullChanged_b()
{
    var selected = jQuery('#submit_full_b').val();
    var text_input = jQuery('.aichatsubmitbut');
    var speech_input = jQuery('.openai-chat-speech-button');
    if(selected != undefined && text_input !== null)
    {
        var customized_chatbot = jQuery('#customized_chatbot').text();
        if(customized_chatbot !== undefined)
        {
            customized_chatbot = customized_chatbot.replace(/submit_full="([^"]*?)"/g, 'submit_full="' + selected.replace('"', "'") + '"');
            jQuery('#customized_chatbot').text(customized_chatbot);
        }
        if (selected == 'on') {
            text_input.css('width', '100%');
        } else {
            text_input.css('width', '');
        }
        if(speech_input !== null)
        {
            if (selected == 'on') {
                speech_input.css('width', '100%');
            } else {
                speech_input.css('width', '');
            }
        }
    }
}
        ...
        success: function (res)
        {
            if(res.status === 'success')
            {
                try 
                {
                    ...
                    if(jsobj.model)
                    {
                        jQuery("#chat_model").val(jsobj.model);
                    }
...
function pageLoadChanged()
{
    var selected = jQuery('#enable_front_end').val();
    if(selected != 'off' && jQuery("#page_load_chat").is(':checked'))
    {
        jQuery('.hideLoad').show();
    }
    else
    {
...
function pageExitChanged()
{
    var selected = jQuery('#enable_front_end').val();
    if(selected != 'off' && jQuery("#page_exit_chat").is(':checked'))
    {
        jQuery('.hideExit').show();
    }
    else
    {
        jQuery('.hideExit').hide();
    }
}
function pageScrollChanged()
{
    var selected = jQuery('#enable_front_end').val();
    if(selected != 'off' && jQuery("#page_scroll_chat").is(':checked'))
    {
        jQuery('.hideScroll').show();
    }
    else
    {
        jQuery('.hideScroll').hide();
    }
}
function pageInactiveChanged()
{
    var selected = jQuery('#enable_front_end').val();
    if(selected != 'off' && jQuery("#page_inactive_chat").is(':checked'))
    {
        jQuery('.hideInactive').show();
    }
    else
    {
        jQuery('.hideInactive').hide();
    }
}
...
function godaiChanged()
{
    if(jQuery("#god_mode_ai").is(':checked'))
    {
        jQuery('.hide_reasoning').show();
    }
    else
    {
        jQuery('.hide_reasoning').hide();
    }
}
function extensionsChanged()
{
    if(jQuery("#god_mode_enable_omniblock").is(':checked'))
    {
        jQuery('.hide_omniblock').show();
    }
    else
    {
        jQuery('.hide_omniblock').hide();
    }
    if(jQuery("#god_mode_enable_database").is(':checked'))
    {
        jQuery('.hide_database').show();
    }
    else
    {
        jQuery('.hide_database').hide();
    }
    ...
    if(jQuery("#god_mode_enable_end").is(':checked'))
    {
        jQuery('.hide_end').show();
    }
    else
    {
        jQuery('.hide_end').hide();
    }
    if(jQuery("#god_mode_enable_chart").is(':checked'))
    {
    ...
    if(jQuery("#god_mode_enable_toubiz").is(':checked'))
    {
        jQuery('.hide_toubiz').show();
    }
    else
    {
        jQuery('.hide_toubiz').hide();
    }
...
jQuery(document).ready(function($)
{
    ...
    jQuery(".aiomatic_convert_persona").on('click', function(e) 
    {
        e.preventDefault();
        if(confirm('Are you sure you want to duplicate this persona?'))
        {
            ...
            else
            {
                e.preventDefault();
                var data = {
                    action: 'aiomatic_convert_persona',
                    personaid: personaid,
                    nonce: aiomatic_object.nonce,
                };
                jQuery.ajax({
                    ...
                    type: 'POST',
                    beforeSend: function (){
                        aiomaticDisable(jQuery('#aiomatic_convert_persona_' + personaid));
                    },
                    success: function (res){
                        if(res.status === 'success'){
                            alert('Persona successfully converted into an AI Assistant. Check the "AI Assistants" menu for details.');
                            aiomaticEnable(jQuery('#aiomatic_convert_persona_' + personaid));
                        }
                        else{
                            alert(res.msg);
                            location.reload();
                    ...
                    error: function (r, s, error){
                        alert('Error in processing persona convertion: ' + error);
                        location.reload();
                    }
                });
            }

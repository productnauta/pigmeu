...
function aiomatic_type_changed() {
    var value = jQuery('#aiomatic-type').children(":selected").attr("value");
    if(value == 'text') {
        jQuery('.hide-when-not-text').show();
        jQuery('.hide-omniblock-form').hide();
    }
    else if(value == 'omniblock-form') {
        jQuery('.hide-when-not-text').hide();
        jQuery('.hide-omniblock-form').show();
    }
    else {
        jQuery('.hide-when-not-text').hide();
        jQuery('.hide-omniblock-form').hide();
...
jQuery(document).ready(function ($)
{
    $(document).on('change','.aiomatic-create-template-field-type', function(e){
        ...
        else if(type === 'color' || type === 'date' || type === 'time' || type === 'datetime' || type === 'captcha' || type === 'month' || type === 'week'){
    ...
    $('body').on("click", '#aiomatic-form-save-button_edit',function(e)
    {
        ...
        if(title === '' || prompt === '' || submit == ''){
            ...
            $.ajax({
                ...
                error: function (r, s, error){
                    aiomaticRmLoading(btn);
                    alert('Error while processing form saving: ' + error);
                }
            });
        }
        return false;
    ...
    var aiomatic_history_form = $('.aiomatic_history_form');
    aiomatic_history_form.on('click', function (){
        if(!aiomaticAjaxRunning) {
            var btn = $(this);
            var id = btn.attr('data-id');
            $.ajax({
                url: aiomatic_object.ajax_url,
                data: {action: 'aiomatic_history_form', id: id, nonce: aiomatic_object.nonce},
                type: 'POST',
                beforeSend: function () {
                    aiomaticAjaxRunning = true;
                    $('.aiomatic_modal_content').html('');
                ...
                error: function (r, s, error) {
                    aiomaticAjaxRunning = false;
                    aiomaticRmLoading(btn);
                    alert('Error in processing history form previewing: ' + error);
                }
            });
        }
    });
    ...
    $('body').on("change", '#aiomatic-edit-type', function(e){
        ...
        if(value == 'text')
        {
            ...
            $('.aiomatic-hide-omniblock-form').hide();
        }
        else
        {
            if(value == 'omniblock-form')
            {
                $('.aiomatic-hide-not-text').hide();
                $('.aiomatic-hide-omniblock-form').show();
            }
            else
            {
                $('.aiomatic-hide-not-text').hide();
    ...
    $('body').on("change", '#aiomatic-type', function(e){
        aiomatic_type_changed();
    });

    aiomatic_type_changed();
    function loadHistoryPage(formId, page) {
        $.ajax({
            url: aiomatic_history_ajax_object_forms.ajax_url,
            method: 'POST',
            data: {
                action: 'aiomatic_history_form',
                nonce: aiomatic_history_ajax_object_forms.nonce,
                id: formId,
                page: page,
            },
            beforeSend: function () {
                $('#aiomatic-history-container').html('<p>Loading...</p>');
            },
            success: function (response) {
                $('#aiomatic-history-container').html(response);

                // Update URL without page reload
                const newUrl = new URL(window.location.href);
                newUrl.searchParams.set('history_page', page);
                window.history.pushState({ formId, page }, '', newUrl);
            },
            error: function () {
                $('#aiomatic-history-container').html('<p>Error loading history. Please try again.</p>');
            },
        });
    }

    $(document).on('click', '.aiomatic-pagination-button', function () {
        const formId = $(this).data('form-id');
        const page = $(this).data('page');
        loadHistoryPage(formId, page);
    });

    window.onpopstate = function (event) {
        if (event.state) {
            loadHistoryPage(event.state.formId, event.state.page);
        }
    };

    const urlParams = new URLSearchParams(window.location.search);
    const initialPage = urlParams.get('history_page') || 1;
    const formId = $('#aiomatic-history-container').data('form-id');
    if (formId) {
        loadHistoryPage(formId, initialPage);
    }
});
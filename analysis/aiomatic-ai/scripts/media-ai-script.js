...
{
    var aimodels = '<option value="openai" selected>OpenAI Dall-E 2</option><option value="dalle3">OpenAI Dall-E 3</option><option value="dalle3hd">OpenAI Dall-E 3 HD</option><option value="gpt-image-1">gpt-image-1</option>';
    ...
    if(aiomatic_img_ajax_object.no_ideogram != '1')
    {
        aimodels += '<option value="ideogram">Ideogram</option>';
    }
    if(aiomatic_img_ajax_object.no_google != '1')
    {
        aimodels += '<option value="google">Google</option>';
    }
    var html = `
...
.wp-block img.aiomatic_ai_image_response_royalty {
...
{
    const resolutions = {
        stable: ['3840x2176', '2560x1472', '1920x1088', '1280x704', '832x512', '512x512', '1152x896', '1216x832', '1344x768', '1536x640', '640x1536', '768x1344', '832x1216', '896x1152'],
        openai: ['256x256', '512x512'],
        dalle3: ['1792x1024', '1024x1792'],
        dalle3hd: ['1792x1024', '1024x1792'],
        'gpt-image-1': ['1792x1024', '1024x1792']
    };

    const defaultResolutions = ['1024x1024']; 
    const selectedModel = jQuery('#aiomatic-ai-modifier').find(":selected").val();

    const modelResolutions = resolutions[selectedModel] || [];

    const allowedResolutions = [...defaultResolutions, ...modelResolutions];

    jQuery('#aiomatic-size-modifier').empty();

    allowedResolutions.forEach(resolution => {
        jQuery('#aiomatic-size-modifier').append(
            jQuery('<option>', { value: resolution, text: resolution })
        );
    });
}
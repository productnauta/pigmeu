...
function openaiimagefunct(uniqid) {
    ...
    var aiomatic_image_ajax_object = window["aiomatic_image_ajax_object" + uniqid];
...
function submitAiomaticImageEditor(uniqid) {
    jQuery('#aiimagesubmitbut' + uniqid).attr('disabled', true);
    var instructionx = jQuery('#aiomatic_image_instruction' + uniqid);
    var instruction = instructionx.val();
    if(instruction == '')
    ...
    var user_token_cap_per_day = aiomatic_image_ajax_object.user_token_cap_per_day;
    var user_id = aiomatic_image_ajax_object.user_id;
    var aiomatic_image_strength = jQuery('#aiomatic_image_strength').val();
    // Show the loading animation
    jQuery('#openai-image-response' + uniqid).html('<div class="automaticx-dual-ring"></div>');
    if(image_size == 'default' || image_size == '')
    {
    ...
    formData.append('action', 'aiomatic_image_editor_ajax_submit');
    formData.append('nonce', aiomatic_image_ajax_object.nonce);
    formData.append('instruction', instruction);
    formData.append('image_size', image_size);
    formData.append('image_model', image_model);
    formData.append('user_id', user_id);
    formData.append('user_token_cap_per_day', user_token_cap_per_day);
    formData.append('aiomatic_image_strength', aiomatic_image_strength);

    const imageFile = jQuery('#aiomatic_image_upload')[0].files[0];
    if (!imageFile) {
    ...
    jQuery.ajax({
        ...
        success: function(response) 
        {
            ...
            if(response.status == 'success')
            {
                ...
                {
                    jQuery("#aiomatic_image_response" + uniqid).attr("src", "data:image/png;base64," + response.data).fadeIn();
                    jQuery('#openai-image-response' + uniqid).html('');
                }
            }
            else

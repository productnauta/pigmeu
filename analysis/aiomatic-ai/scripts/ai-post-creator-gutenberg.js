...
jQuery(document).ready(function ($)
{
    ...
    function aiomaticSaveProduct(post_id, aiomatic_title, aiomatic_ai_seo, aiomatic_ai_seo_title, aiomatic_ai_content, aiomatic_ai_excerpt, aiomatic_ai_tags, aiomatic_ai_focus)
    {
        var data = {'action': 'aiomatic_save_post_ai', 'post_id': post_id, 'aiomatic_title': aiomatic_title, 'aiomatic_ai_seo': aiomatic_ai_seo, 'aiomatic_ai_seo_title': aiomatic_ai_seo_title, 'aiomatic_ai_focus': aiomatic_ai_focus, 'aiomatic_ai_content': aiomatic_ai_content, 'aiomatic_ai_excerpt': aiomatic_ai_excerpt, 'aiomatic_ai_tags': aiomatic_ai_tags, 'nonce': aiomatic_creator_object.nonce}
    ...
    function aiomaticProductGenerator(title, step, steps)
    {
        ...
        if( $('#ai_title_vision').length )
        {
            var ai_title_vision = $('#ai_title_vision').prop('checked') ? 1 : 0;
        }
        else
        {
            var ai_title_vision = 0;
        }
        ...
        if( $('#aiomatic_seo_title_prompt').length )
        {
            var seo_title = $('#aiomatic_seo_title_prompt').val();
        }
        else
        {
            var seo_title = '';
        }
        if( $('#aiomatic_focus_prompt').length )
        {
            var focus = $('#aiomatic_focus_prompt').val();
        }
        else
        {
            var focus = '';
        ...
        if(aiomatic_step === 'metatitle')
        {
            if( $('#ai-generator-status').length )
            {
                $('#ai-generator-status').html(aiomatic_creator_object.generating_meta_title);
            }
        }
        ...
        if(aiomatic_step === 'focus')
        {
            if( $('#ai-generator-status').length )
            {
                $('#ai-generator-status').html(aiomatic_creator_object.generating_focus);
            }
        }
        $.ajax({
            url: aiomatic_creator_object.ajax_url,
            ...
            success: function (res){
                if(res.success == true)
                {
                    ...
                    if(aiomatic_step === 'metatitle')
                    {
                        if( $('#aiomatic_ai_seo_title').length )
                        {
                            var curtag = $('#aiomatic_ai_seo_title').val();
                            if(curtag == '')
                            {
                                $('#aiomatic_ai_seo_title').val(res.data.content);
                            }
                            else
                            {
                                $('#aiomatic_ai_seo_title').val(curtag + " " + res.data.content);
                            }
                        }
                    }
                    if(aiomatic_step === 'description')
                    ...
                    if(aiomatic_step === 'focus')
                    {
                        if( $('#aiomatic_ai_focus').length )
                        {
                            var curtag = $('#aiomatic_ai_focus').val();
                            if(curtag == '')
                            {
                                $('#aiomatic_ai_focus').val(res.data.content);
                            }
                            else
                            {
                                $('#aiomatic_ai_focus').val(curtag + "," + res.data.content);
                            }
                        }
                    }
    ...
    aiomatic_ai_content_generator_btn.on('click', function ()
    {
        ...
        if( $('#aiomatic_generate_meta_title').length )
        {
            var aiomatic_generate_meta_title = $('#aiomatic_generate_meta_title').prop('checked') ? 1 : 0;
        }
        else
        {
            var aiomatic_generate_meta_title = 0;
        }
        ...
        if( $('#aiomatic_generate_focus').length )
        {
            var aiomatic_generate_focus = $('#aiomatic_generate_focus').prop('checked') ? 1 : 0;
        }
        else
        {
            var aiomatic_generate_focus = 0;
        }
        var aiomaticSteps = [];
        ...
        if(aiomatic_generate_meta_title)
        {
            aiomaticSteps.push('metatitle');
        }
        ...
        if(aiomatic_generate_focus)
        {
            aiomaticSteps.push('focus');
        }
        if(aiomatic_title === '')
        {
    ...
    aiomatic_save_ai_content_btn.on('click', function ()
    {
        ...
        if( $('#aiomatic_ai_seo_title').length )
        {
            var aiomatic_ai_seo_title = $('#aiomatic_ai_seo_title').val();
        }
        else
        {
            var aiomatic_ai_seo_title = '';
        }
        if( $('#aiomatic_ai_content').length )
        {
        ...
        if( $('#aiomatic_ai_focus').length )
        {
            var aiomatic_ai_focus = $('#aiomatic_ai_focus').val();
        }
        else
        {
            var aiomatic_ai_focus = '';
        }
        if(aiomatic_title == '' && aiomatic_ai_seo == '' && aiomatic_ai_seo_title == '' && aiomatic_ai_content == '' && aiomatic_ai_excerpt == '' && aiomatic_ai_tags == '' && aiomatic_ai_focus == '')
        {
        ...
        else
        {
            ...
            {
                aiomaticSaveProduct(aiomatic_creator_object.post_id, aiomatic_title, aiomatic_ai_seo, aiomatic_ai_seo_title, aiomatic_ai_content, aiomatic_ai_excerpt, aiomatic_ai_tags, aiomatic_ai_focus);
            }
            else
            {
